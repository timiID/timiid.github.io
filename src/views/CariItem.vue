...
@click="toggleMonsterLocation(item.id, mmi)"
:class="['cursor-pointer grid grid-cols-[auto_1fr_auto] pl-9 pr-5 py-2 items-center border-b gap-3 text-sm',
isDark ? 'border-white/5 hover:bg-white/[0.02]' : 'border-slate-100 hover:bg-slate-50']"
>
...
<span @click.stop="bukaModalMonster(monster)" :class="['font-semibold cursor-pointer hover:underline', isDark ? 'text-slate-200' : 'text-slate-800']">{{ monster.name }}</span>
<span v-if="monster.level !== '?'" :class="['text-xs tabular-nums', isDark ? 'text-slate-500' : 'text-slate-400']">
Lv.{{ monster.level }}
</span>
</div>
<div v-if="activeMonster.itemId === item.id && activeMonster.mi === mmi" :class="['px-12 py-2 bg-white/5 rounded-b-lg text-sm', isDark ? 'bg-white/[0.02]' : 'bg-slate-50']">
<div class="flex items-center justify-between gap-3">
<div class="text-xs">
<div class="font-bold">Location</div>
<div class="text-[13px]" :class="isDark ? 'text-slate-300' : 'text-slate-700'">{{ monster.map }}</div>
<div class="text-[12px] opacity-70">Lv. {{ monster.level }} · {{ monster.type }}</div>
</div>
<div class="flex items-center gap-2">
<button @click.stop="toggleMapView(item.id)" class="px-3 py-1 rounded-xl bg-indigo-600 text-white text-xs font-bold">Show on Map</button>
<button @click.stop="toggleMonsterLocation(item.id, mmi)" class="px-3 py-1 rounded-xl bg-slate-200 text-xs">Close</button>
</div>
</div>
</div>
</template>
...
@click="toggleMonsterLocation(item.id, mi)"
:class="['cursor-pointer grid grid-cols-[1fr_auto] px-5 py-2.5 items-start border-b text-sm',
isDark ? 'border-white/5 hover:bg-white/[0.02]' : 'border-slate-100 hover:bg-slate-50']"
>
<div>
<span @click.stop="bukaModalMonster(monster)" :class="['font-semibold cursor-pointer hover:underline', isDark ? 'text-slate-200' : 'text-slate-800']">{{ monster.name }}</span>
...
<div v-if="activeMonster.itemId === item.id && activeMonster.mi === mi" :class="['mt-1 px-5 py-2 bg-white/5 rounded-b-lg text-sm', isDark ? 'bg-white/[0.02]' : 'bg-slate-50']">
<div class="flex items-center justify-between">
<div>
<div class="text-xs font-bold">Location</div>
<div :class="['text-[13px] font-semibold', isDark ? 'text-slate-300' : 'text-slate-700']">{{ monster.map }}</div>
<div class="text-[12px] opacity-70">Lv. {{ monster.level }} · {{ monster.type }}</div>
</div>
<div class="flex items-center gap-2">
<button @click.stop="toggleMapView(item.id)" class="px-3 py-1 rounded-xl bg-indigo-600 text-white text-xs font-bold">Show on Map</button>
<button @click.stop="toggleMonsterLocation(item.id, mi)" class="px-3 py-1 rounded-xl bg-slate-200 text-xs">Close</button>
</div>
</div>
</div>
</div>
...
<Transition name="modal-fade">
<div v-if="monsterModal.open" class="fixed inset-0 z-[210] flex items-center justify-center p-4" @click.self="monsterModal.open = false">
<div class="absolute inset-0 bg-black/70 backdrop-blur-sm"></div>
<div :class="['relative w-full max-w-3xl rounded-2xl border shadow-2xl overflow-hidden', isDark ? 'bg-slate-900 border-white/10' : 'bg-white border-slate-200']">
<div :class="['px-6 py-4 border-b flex items-center justify-between gap-3', isDark ? 'border-white/8 bg-white/[0.02]' : 'border-slate-200 bg-slate-50']">
<div>
<p :class="['text-[10px] font-black uppercase tracking-widest mb-0.5', isDark ? 'text-violet-400' : 'text-violet-600']">Monster Details</p>
<h3 :class="['text-base font-bold m-0 leading-tight', isDark ? 'text-white' : 'text-slate-900']">{{ monsterModal.data?.name || 'Monster' }}</h3>
</div>
<button @click="monsterModal.open = false" :class="['w-8 h-8 rounded-lg flex items-center justify-center transition-colors shrink-0', isDark ? 'text-slate-400 hover:bg-white/8 hover:text-white' : 'text-slate-400 hover:bg-slate-100 hover:text-slate-700']">
<svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
<path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5" d="M6 18L18 6M6 6l12 12"/>
</svg>
</button>
...
<div v-if="monsterModal.loading" class="py-8 text-center">
<svg class="w-7 h-7 animate-spin text-violet-400" fill="none" viewBox="0 0 24 24"><circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"/><path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8v8z"/></svg>
</div>

<div v-else-if="monsterModal.error" class="py-6 text-center text-red-400">{{ monsterModal.error }}</div>

<div v-else-if="monsterModal.data">
<div class="grid grid-cols-2 gap-4 mb-4">
<div>
<div class="text-xs font-black uppercase">Lv</div>
<div class="font-bold">{{ monsterModal.data.level }}</div>
</div>
<div>
<div class="text-xs font-black uppercase">Type</div>
<div class="font-bold">{{ monsterModal.data.type_label || monsterModal.data.type || '—' }}</div>
</div>
</div>

<div class="mb-4">
<div class="text-xs font-black uppercase">Spawn at</div>
<div class="font-semibold text-[14px]">{{ monsterModal.data.map_name || monsterModal.data.spawn || 'Unknown' }}</div>
</div>

<div>
<div class="text-sm font-black uppercase tracking-wider mb-2">Item Drops</div>
<div v-if="(monsterModal.data.drops && monsterModal.data.drops.length) || (monsterModal.data.items && monsterModal.data.items.length)">
<ul class="list-none m-0 p-0 space-y-2">
<li v-for="(d, di) in (monsterModal.data.drops || monsterModal.data.items || [])" :key="di" class="text-sm">
<span :class="['font-medium', isDark ? 'text-slate-300' : 'text-slate-800']">{{ d.name || d.item || d.title || d }}</span>
</li>
</ul>
</div>
<div v-else class="text-sm italic opacity-60">No drop data available.</div>
</div>
</div>
</div>
</div>
