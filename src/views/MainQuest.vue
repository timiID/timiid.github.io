...
const filteredItems = computed(() => {
    return mqData.filter(item => {
        ...
        // Cek pencarian di kolom name_id, name_en, dan chapter
        const matchSearch = 
            (item.name_id && item.name_id.toLowerCase().includes(s)) || 
            (item.name_en && item.name_en.toLowerCase().includes(s)) || 
            (item.chapter && item.chapter.toLowerCase().includes(s));
                ...
                <div :class="['lg:hidden divide-y-[3px]', props.isDark ? 'divide-white/10' : 'divide-slate-200']">
                    <div v-for="(item, index) in paginatedItems" :key="index" class="p-6 space-y-4">
                        ...
                        <div :class="['flex justify-between items-center p-5 rounded-2xl border-[3px]', 
                            ...
                            <div class="font-black uppercase text-sm leading-tight max-w-[60%]">
                                {{ locale === 'id' ? item.name_id : item.name_en }}
                                <span class="block text-[9px] opacity-40 font-bold italic normal-case tracking-normal">
                                    {{ locale === 'id' ? item.name_en : item.name_id }}
                                </span>
                            </div>
